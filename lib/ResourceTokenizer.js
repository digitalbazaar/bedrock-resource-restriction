...
export class ResourceTokenizer {
  ...
  async process({acquisitionRecord, now = Date.now()} = {}) {
    ...
    this._requiredTokenizerIds = requiredTokenizerIds;
    ...
    // add maps and hmacs for all required tokenizer IDs
    const getHmacPromises = [];
    for(const tokenizerId of requiredTokenizerIds) {
      if(!this.maps.has(tokenizerId)) {
        ...
        getHmacPromises.push((async () => {
          const {hmac} = await tokenizers.get({id: tokenizerId});
          this.hmacs.set(tokenizerId, hmac);
        })());
      }
    }
    await Promise.all(getHmacPromises);

    // tokenize all resources in the given `request` (`resourceIds`)
    const {resourceIds} = this;
    await this._updateTokenizerMaps({resourceIds});
  }
   ...
   *
   * @param {object} options - Options to use.
   * @param {Array} [options.resourceIds=this.resourceIds] - A list of
   *   resource IDs to get the acquisition information for; if not given, all
   *   the resource IDs in the `request` associated with this
   *   `ResourceTokenizer` will be used.
   *
   * @returns {Promise<Map>} The map of untokenized
   *   resource IDs => [{count, requested}].
   */
  async getUntokenizedAcquisitionMap({resourceIds} = {}) {
    // convert tokenized resources tracked in `acquisitionRecord` into a map
    // currently acquired resources applicable to `resourceIds` (or the
    // resources in `request`, `resourceIds` was not passed) using keys of
    // non-tokenized resource identifiers and values of `{count, requested}`
    const acquired = new Map();
    const {prunedTokenized} = this;
    if(!resourceIds) {
      resourceIds = this.resourceIds;
    } else {
      // update tokenizer maps to ensure they have the specified resource IDs
      await this._updateTokenizerMaps({resourceIds});
    }
    for(const resourceId of resourceIds) {
      // `resource` appears (tokenized) in one or fewer entries in `tokenized`
      for(const {tokenizerId, resources: resourceMap} of prunedTokenized) {
        ...
        const list = resourceMap[tokenizedId];
        if(list) {
          acquired.set(resourceId, list);
        }
      }
    }
    return acquired;
  ...
  applyAcquireRequest({checkResults} = {}) {
    ...
    const {trackedResources, maxRestrictionTtl} = checkResults;
    ...
    for(const {resource, count, requested} of prunedRequest) {
      if(!trackedResources.has(resource)) {
        // resource not tracked, so not acquired
        continue;
      }
  ...
  async _updateTokenizerMaps({resourceIds}) {
    // update all tokenizer maps in parallel
    const promises = [];
    const {acquirerId, hmacs, maps, reverseMaps} = this;
    for(const [tokenizerId, map] of maps) {
      const reverseMap = reverseMaps.get(tokenizerId);
      const hmac = hmacs.get(tokenizerId);
      for(const resourceId of resourceIds) {
        // already mapped
        if(map.get(resourceId)) {
          continue;
        }

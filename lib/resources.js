...
export async function check(
  ...
  const status = await _createStatus({excessResources});
  return {authorized, excessResources, untrackedResources, status};
}
...
async function _createStatus({excessResources}) {
  const status = {
  ...
  if(excessResources.length === 0) {
    ...
    status.error = 'ERRCD_OK';
  } else {
    status.result = 'failure';
    status.error = 'ERRCD_NOTALLOWED';
  }
  return status;
}

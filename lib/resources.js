...
export async function check(
  ...
  const checkResults = await _check({request, zones, resourceTokenizer});
...
async function _check({request, zones, resourceTokenizer} = {}) {
  ...
  for(const restriction of restrictions) {
    const result = restriction.apply({acquirerId, acquired, request, zones});

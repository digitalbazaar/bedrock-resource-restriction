 ...
 */
export async function check({acquirerId, request, factors} = {}) {
  // 1. Get the acquisition record associated with `acquirerId`.
  const acquisitionRecord = await _getAcquisitionRecord({acquirerId});

  // 2. Create map of external resource IDs to pairwise resource IDs.
  const resourceIds = request.map(e => e.resource);
  const resourceMapper = new ResourceMapper({resourceIds});
  await resourceMapper.compute({acquisitionRecord});

  // 3. Run internal check helper to see if acquisition is possible.
  const checkResults = await _check(
    {acquirerId, request, factors, resourceMapper});
  ...
  return {
    ...
    ...untrackedResources && {untrackedResources}
  };
}
...
export async function acquire(
  {acquirerId, request, factors, forceAcquisition = false} = {}) {
  ...
  while(true) {
    // 3. Run internal check helper to see if acquisition is possible.
    const checkResults = await _check(
      {acquisitionRecord, request, factors, resourceMapper});
    const {authorized, excessResources, untrackedResources} = checkResults;
    ...
    if(await _record({acquirerId, request, checkResults})) {
    ...
    // 8. Recompute the resource maps as the new acquisition record may have
    //   reference new tokenizers or the current tokenizer may have been
    //   rotated.
    await resourceMapper.compute({acquisitionRecord});
  }
}
...
export async function release({acquirerId, request} = {}) {
  ...
  // TODO: implement timeout
  while(true) {
    // 3. Build new `tokenized` entry for acquisition record from `request`,
    //   old `tokenized` record and `resourceMapper`.
    const {acquisition: {tokenized}} = acquisitionRecord;
    const {newTokenized, excessResources} = _removeRequest(
      {request, tokenized, resourceMapper});
    ...
    if(newTokenized.length === 1 &&
      ...
      if(await _upsertAcquisitionRecord(
        {acquirerId, acquisitionRecord, newTokenized})) {
        // recording successful, return relevant results
        return {excessResources};
      }
    }
    ...
    // 7. Recompute the resource maps as the new acquisition record may have
    //   reference new tokenizers or the current tokenizer may have been
    //   rotated.
    await resourceMapper.compute({acquisitionRecord});
  }
...
async function _check(
  ...
  const acquired = _getAcquired({acquisitionRecord, resourceMapper});
  ...
  for(const restriction of restrictions) {
    ...
    // all restrictions must be authorized or check not authorized
    authorized &= result.authorized;
    // aggregate all tracked resources
...
async function _record({acquirerId, request, checkResults} = {}) {
  ...
  const {acquisitionRecord, resourceMapper} = checkResults;

  // build new `tokenized` entry for acquisition record from `request`, old
  // `tokenized` record and `resourceMapper`
  const {acquisition: {tokenized}} = acquisitionRecord;
  const {newTokenized} = _addRequest({request, tokenized, resourceMapper});
...
function _getAcquired({acquisitionRecord, request, resourceMapper}) {
  // convert tokenized resources tracked in `acquisitionRecord` into a map of
  // currently acquired resources applicable to `request` using non-tokenized
  const acquired = new Map();
  const {tokenized} = acquisitionRecord.acquisition;
  for(const {resource} of request) {
    for(const {tokenizerId, resources} of tokenized) {
      const reverseMap = resourceMapper.getReverseMap({tokenizerId});
      const tokenizedId = reverseMap.get(resource);
      // a given resource can only appear in one `tokenized` entry, so there
      // is no need to total them; add resources marked as acquired in
      // `tokenized` to total in `acquired` using the `resource` identifier
      const count = resources[tokenizedId] || 0;
      // FIXME: needs to be {acquired: <string/number>, count: <number>}
      acquired.set(resource, count);
    }
  }
  return acquired;
}

function _addRequest({request, tokenized, resourceMapper}) {
  const {newTokenizerId} = resourceMapper;
  const newTokenized = _createNewTokenizedAcquisition(
    {tokenized, resourceMapper});
  const entry = newTokenized[1] || newTokenized[0];

  // add new resources from request
  const tokenizeMap = resourceMapper.getTokenizeMap(
  ...
  for(const {resource, count, requested} of request) {
    ...
    list.splice(i, 0, {resource: tokenizedId, count, requested});
  }

  return {newTokenized};
}

function _removeRequest({request, tokenized, resourceMapper}) {
  const {newTokenizerId} = resourceMapper;
  const newTokenized = _createNewTokenizedAcquisition(
    {tokenized, resourceMapper});
  const entry = newTokenized[1] || newTokenized[0];

  // remove resources expressed in request, oldest first; dedupe resources
  // before removing them
  const tokenizeMap = resourceMapper.getTokenizeMap(
    {tokenizerId: newTokenizerId});
  const excessResources = new Map();
  for(const {resource, count} of request) {
    const tokenizedId = tokenizeMap.get(resource);
    const list = entry.resources[tokenizedId] || [];
    // entries are sorted, remove entries starting from beginning until
    // `count` is fully consumed
    let toRemove = count;
    const newList = [];
    for(const e of list) {
      if(toRemove >= e.count) {
...
function _createNewTokenizedAcquisition({tokenized, resourceMapper}) {
  ...
  const {rotate, newTokenizerId} = resourceMapper;
  let newTokenized = [];
  let entry;
  if(!rotate) {
    ...
    const unconverted = {
      tokenizedId: tokenized[0].tokenizedId,
      resources: {}
    };
    for(const {tokenizerId, resources} of tokenized[0]) {
      const untokenizeMap = resourceMapper.getUntokenizeMap({tokenizerId});
      const tokenizeMap = resourceMapper.getTokenizeMap(
        {tokenizerId: newTokenizerId});

...
import {paginateGetResult, getCount} from './helpers.js';
 ...
 * @param {string} options.offset - The number of documents to skip in a query
 *   result.
 * @param {boolean} [options.explain=false] - An optional explain boolean.
 *
 * @returns {Promise<object | ExplainObject>} Resolves with an object with the
...
export async function getAll({
  zone, resource, limit = 10, offset = 0, explain = false
} = {}) {
  const query = {

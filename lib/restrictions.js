...
export async function insert({restriction} = {}) {
  await validateSchema({restriction});
  ...
  if(restriction.method === 'limitOverDuration') {
    const duration = restriction.methodOptions.duration;
    id = `${id}${duration}`;
    // pattern is ISO8601 regex
    if(!(duration && pattern.test(duration))) {
      throw new BedrockError(
        `Duration "${duration}" is not valid.`,
        'ValidationError', {
          public: true,
          httpStatusCode: 400
        });
  ...
  record.restriction.id =
    crypto.createHash('sha256').update(id).digest('base64');
  try {
...
export async function bulkInsert({restrictions} = {}) {
  ...
  const now = Date.now();
  const meta = {created: now, updated: now};
  let records = restrictions.map(function(restriction) {
    let id = `${restriction.zone}${restriction.resource}${restriction.method}`;
    if(restriction.method === 'limitOverDuration' &&
    restriction.methodOptions && restriction.methodOptions.duration) {
      id = `${id}${restriction.methodOptions.duration}`;
    }
    restriction.id = crypto.createHash('sha256').update(id).digest('base64');
    return {restriction, meta};
  });

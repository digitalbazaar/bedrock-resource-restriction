 ...
 *
 * @returns {Promise | ExplainObject} Settles once the operation completes or
 *   returns an ExplainObject if `explain=true`.
 */
 ...
 * @param {boolean} [options.explain=false] - An optional explain boolean.
 *
 * @returns {Promise<object> | ExplainObject} Returns an object with the record
 ...
 * @param {boolean} [options.explain=false] - An optional explain boolean.
 *
 * @returns {Promise<object> | ExplainObject} Returns an object with the
 *   `restrictions` property which is an array of matching records or returns
 *   an ExplainObject if `explain=true`.
 */
export async function getAll({zone, resource, explain = false} = {}) {
  const query = {
 ...
 * @param {boolean} [options.explain=false] - An optional explain boolean.
 *
 * @returns {Promise<object> | ExplainObject} Returns an object with
 *   `restrictions` with an array of applicable `ResourceRestriction` instances
 *   to be applied to the request or returns an ExplainObject if `explain=true`.
 */
...
async function _limitOverDuration(
  ...
  const startTime = moment().subtract(parsedDuration).unix() * 1000;

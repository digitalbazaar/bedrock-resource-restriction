...
bedrock.events.on('bedrock-mongodb.ready', async () => {
  ...
  await promisify(database.createIndexes)([{
    ...
    fields: {'restriction.resourceId': 1, 'restriction.zoneId': 1},
    options: {unique: true, background: false}
  }, {
    // for getting all restrictions in a zone
    collection: 'resource-restriction-restriction',
    fields: {'restriction.zoneId': 1},
    options: {unique: true, background: false}
  }]);
 ...
 * @param {string} options.restriction.zoneId - The ID of the zone that the
 *   restriction applies to.
 * @param {string} options.restriction.resourceId - The ID of the resource that
...
export async function update({restriction} = {}) {
  const query = {
    'restriction.zoneId': restriction.zoneId,
    'restriction.resourceId': restriction.resourceId
  };
  const collection = database.collections['resource-restriction-restriction'];
  const $set = {
 ...
 *
 * @param {object} options - Options to use.
 * @param {string} options.zoneId - The ID of the zone that the
 *   restriction applies to.
 * @param {string} options.resourceId - The ID of the resource that the
 *   restriction applies to.
 *
 * @returns {Promise<object>} An object with the record.
 */
export async function get({zoneId, resourceId} = {}) {
  const query = {
    'restriction.zoneId': zoneId,
    'restriction.resourceId': resourceId
  };
...
export async function matchRequest({request, zones} = {}) {
  ...
  const query = {
    'restriction.zoneId': {$in: zones},
    'restriction.resourceId': {$in: resourceIds}
  };
  const projection = {_id: 0};
  const collection = database.collections['resource-restriction-restriction'];
 ...
 *   signature:
 *   Promise<{authorized, trackedResources, excessResources}> method(
 *     {acquired, request, zones})
 */
...
// TODO: add built-in method that checks limits over a period

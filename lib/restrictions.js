...
import {
  formatQuery, getCount, getLimitAndOffset, paginateGetResult
} from './helpers.js';
 ...
 */
export async function getAll({query, explain = false} = {}) {
  ...
  if(explain) {
    const cursor = await collection.find(formattedQuery, {projection});
    return cursor.explain('executionStats');
  }

  const records = await paginateGetResult({
    limit, offset, collection, query: formattedQuery, sort, projection
  });

  const restrictions = records.map(record => record.restriction);
  const {count} = await getCount({collection, query: formattedQuery});
